...
static bool             pid_precalculations     (p_pid_t pid_inst);
...
*
* @param[in]    p_cfg   - Pointer to controller configuration
* @return       precalculations succeed
*/
////////////////////////////////////////////////////////////////////////////////
static bool pid_precalculations(p_pid_t pid_inst)
{
    ...
    if ( pid_inst->cfg.kd > 0.0f )
    {
        status = pid_rc_calculate_alpha( pid_inst->cfg.lpf_d.fc, (1.0f / pid_inst->cfg.ts), &pid_inst->cfg.lpf_d.alpha );
    }
    else
    {
        pid_inst->cfg.lpf_d.alpha = 1.0f; // bypass
    }
        ...
        *pid_inst = calloc( 1U, sizeof( pid_t ));
        ...
        if ( NULL != *pid_inst )
        {
            ...
            if ( true == pid_check_cfg( p_cfg ))
            {
                ...
                if ( true == pid_precalculations( *pid_inst ))
                {
                    // Set to zero
...
pid_status_t pid_set_cfg(p_pid_t pid_inst, const pid_cfg_t * const p_cfg)
{
	...
	if ( true == pid_inst->is_init )
	{
			...
			&&	( NULL != p_cfg ))
		{
			...
			if ( !pid_precalculations( pid_inst ))
			{
			    status = ePID_ERROR;
			}
		}
...
pid_status_t pid_reset(p_pid_t pid_inst)
{
    ...
    if ( NULL != pid_inst )
    {
        ...
        pid_inst->p_ff_d        = 0.0f;

        // Initialize LPF state to current measurement to avoid startup D spike
        pid_inst->act_filt      = pid_inst->in.act;
        pid_inst->act_filt_prev = pid_inst->in.act;
    }
    else
    {
        status = ePID_ERROR;
